...
import { projects, resumeLink, type Project } from "./constants";
import { getTutorialNextPage, markPageAsViewed } from "./tutorial";
...
const Portfolio = () => {
  ...
  const [currentWindow, setCurrentWindow] = useState<JSX.Element>(
    <TitleWindow />,
  );
  ...
  const [tutorialCommand, setTutorialCommand] = useState<string | undefined>(
    undefined,
  );

  useEffect(() => {
    setTutorialCommand(getTutorialNextPage());
  }, []);
  ...
  const enterCommand = (
    ...
    switch (command.toLowerCase()) {
      ...
      case "whoami":
        ...
        markPageAsViewed("whoami");
        setCurrentWindow(<TitleWindow />);
        break;
      case "resume":
        window.open(resumeLink, "_blank");
        markPageAsViewed("resume");
        break;
      case "help":
        if (pushState)
          history.pushState(
      ...
      default: {
        ...
        if (
          lowerCommandSplit[0] === "projects" &&
          lowerCommandSplit.length > 1
        ) {
  ...
  return (
    <div
      className={
        "portfolio " + (isCookieWindowOpen || viewingProject ? "no-scroll" : "")
      }
    >
      <div className="terminal-container">
        {!closedWindow ? (
          <Terminal
            ...
            isFocused={!isCookieWindowOpen && !viewingProject}
          >
            {currentWindow}
          </Terminal>
        ) : (
          <p style={{ textAlign: "center" }}>
            Oops! You aren't supposed to see this!
          </p>
        )}
      ...
      {isCookieWindowOpen ? (
        <CookieWindow onClose={() => setIsCookieWindowOpen(false)} />
      ) : (
        <FallingCookieSection
          onCookieClick={() => setIsCookieWindowOpen(true)}
        />
      )}
    </div>
  );
